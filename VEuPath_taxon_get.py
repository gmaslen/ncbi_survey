...
def getVEuPathAssemblies() -> list:
    ...
    hm = (
        base_url
        + '?reportConfig={"attributes":["primary_key","genome_source","genome_version","annotation_version","is_reference_strain","ncbi_tax_id"],"tables":[],"attributeFormat":"text"}'
    )
...
def getNCBIrecord(Original_Accn: str, VPassembly: VPassembly) -> list:
    ...
    for assembly in get_assembly_metadata_by_asm_accessions(GCAid):
        ...
        ncbi_tax_id = (
            "unknown"
            if not assembly.get("assembly").get("org").get("tax_id")
            else assembly.get("assembly").get("org").get("tax_id")
        )

        org_assemblies, mismatch_version_meta = getAssembliesByTaxon(
            Original_Accn, ncbi_tax_id
        )
...
def getAssembliesByTaxon(Original_Accn: str, ncbi_tax_id: str) -> list:
    ...
    for assembly in get_assembly_metadata_by_taxon(ncbi_tax_id):
        ...
        total_genes = (
            annotation.get("total_genes") if annotation.get("total_genes") else 0
        )
        total_proteins = (
            annotation.get("total_proteins") if annotation.get("total_proteins") else 0
        )
        ...
        if asm_accession.startswith(
            Original_Accn.split(".")[0]
        ) and not asm_accession.endswith(Original_Accn.split(".")[1]):
            logging.warning(
                f"Query accession {Original_Accn} not returned via API ! [Version obtained: {asm_accession}]"
            )
...
def getAnnoMetadata(assembly: dict) -> dict:
    ...
    if assembly.get("assembly").get("annotation_metadata"):
        ...
        ret.update(
            {"total_genes": annstats.get("total") if annstats.get("total") else 0}
        )
        ret.update(
            {
                "total_proteins": annstats.get("protein_coding")
                if annstats.get("protein_coding")
                else 0
            }
        )
    return ret
...
def getOrgData(assembly: dict) -> dict:
    ...
    if assembly.get("assembly").get("org"):
        ...
        ret.update(
            {"strain": orgdat.get("strain") if orgdat.get("strain") else "unknown"}
        )
...
def checkAccessions(
    user_supplied_acc: ncbi_accession, ncbi_acc: ncbi_accession
) -> list:
    ...
    if user_supplied_acc != ncbi_acc:
        ...
        if user_supplied_acc.id != ncbi_acc.id:
            accession_mismatch.append(
                "ID"
            )  # Assembly GCA/GFC ID GC{CF}_ id -> 'XXXXXXXXX'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate report of VEuPath and NCBI/Refseq assembly overlaps."
    )
    parser.add_argument(
        "--out",
        help="Name of output report html file.",
        default=default_output,
        type=str,
        required=False,
    )
    parser.add_argument(
        "--test", help=f"run in test mode using internal data", action="store_true"
    )
    parser.add_argument(
        "--assemblies", help=f"path of assembly input file", type=str, required=False
    )
    parser.add_argument(
        "--sampleN",
        help=f"number of retrieved assemblies to process",
        type=int,
        required=False,
    )
    ...
    if args.test:
        ...
        tst_busco = VPassembly(
            "GCF_943734845.2", "Anopheles funestus", "yes", "AfunF3.2", "62324"
        )
        tst_strain = VPassembly(
            "GCA_003013265.1", "Trypanosoma congolense", "no", "unknown", "1068625"
        )
    ...
    if args.sampleN:
        ...
        print(
            "Processing with sub-sample enabled. Operating over "
            + str(subsample)
            + " of "
            + str(len(assemblies))
            + " original input assembly queries from file "
            + str(args.assemblies)
        )
    else:
    ...
    for valid_assembly_accn, assembly_vp_object in valid_accessions.items():
        ...
        asm_record, mismatched_meta = getNCBIrecord(
            valid_assembly_accn, assembly_vp_object
        )
        ...
        if mismatched_meta is not None:
            ...
            with open(
                f"{file_prefix}.api_return_version_mismatched.tsv", "a"
            ) as tsv_out:
                writer = csv.writer(tsv_out, delimiter="\t", lineterminator="\n")
